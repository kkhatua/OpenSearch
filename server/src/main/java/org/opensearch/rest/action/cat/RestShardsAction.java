...
public class RestShardsAction extends AbstractListAction {
    ...
    static void applyRoutingTopKPushdown(RestRequest request, CatShardsRequest shardsRequest) {
        ...
        // Aggregation (h=func(...)) regroups rows and applies the limit to GROUPS, not shards.
        // Truncating shards to top-K at the source would compute aggregates over only those K
        // shards and corrupt the result, so skip the routing pushdown when aggregation is requested.
        if (TableSummarizer.hasAggregation(request.param("h"))) {
            return;
        }
